...
////// Emissive:
                ...
                float3 node_1763 = (_LightColor0.rgb*(((node_2552.r*max(0,dot(lightDirection,mul( unity_ObjectToWorld, float4(node_7196,0) ).xyz.rgb)))+(node_2552.g*max(0,dot(lightDirection,mul( unity_ObjectToWorld, float4(float3(0,1,0),0) ).xyz.rgb)))+(node_2552.b*max(0,dot(lightDirection,mul( unity_ObjectToWorld, float4(float3(0,0,1),0) ).xyz.rgb)))+((node_4689.r*max(0,dot(lightDirection,mul( unity_ObjectToWorld, float4(float3(-1,0,0),0) ).xyz.rgb)))+(node_4689.g*max(0,dot(lightDirection,mul( unity_ObjectToWorld, float4(float3(0,-1,0),0) ).xyz.rgb)))+(node_4689.b*max(0,dot(lightDirection,mul( unity_ObjectToWorld, float4(float3(0,0,-1),0) ).xyz.rgb)))))/3.0)*_TranslucencyIntensity*_TranslucencyColor.rgb);
                float3 emissive = node_1763;
                */

                
                float4 _WorldPosOffsetValues = tex2D(_WorldPosOffset,TRANSFORM_TEX(i.uv0, _WorldPosOffset));
                float4 _WorldNegPosOffsetValues = tex2D(_WorldNegPosOffset,TRANSFORM_TEX(i.uv0, _WorldNegPosOffset));

                float3 emissiveSum = (_WorldPosOffsetValues.r*max(0, dot(lightDirection,mul( unity_ObjectToWorld, float4(1,0,0,0)).rgb)));
                emissiveSum += (_WorldPosOffsetValues.g*max(0,dot(lightDirection,mul( unity_ObjectToWorld, float4(0,1,0,0)).rgb)));
                emissiveSum += (_WorldPosOffsetValues.b*max(0,dot(lightDirection,mul( unity_ObjectToWorld, float4(0,0,1,0)).rgb))); 
                emissiveSum += (_WorldNegPosOffsetValues.r*max(0,dot(lightDirection,mul( unity_ObjectToWorld, float4(-1,0,0,0)).rgb))); 
                emissiveSum += (_WorldNegPosOffsetValues.g*max(0,dot(lightDirection,mul( unity_ObjectToWorld, float4(0,-1,0,0)).rgb))); 
                emissiveSum += (_WorldNegPosOffsetValues.b*max(0,dot(lightDirection,mul( unity_ObjectToWorld, float4(0,0,-1,0)).rgb)));  
                float3 emissive = _LightColor0.rgb * (emissiveSum/3.0) *_TranslucencyIntensity*_TranslucencyColor.rgb;
